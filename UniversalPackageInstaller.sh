#!/bin/bash
################################################################################
# UniversalPackageInstaller_Linux (UPI)
# Alias : upi
...
#
# Description :
#   UPI est un script universel d’installation de paquets pour Linux (apt/snap)
#   et Windows (UnigetUI/Winget). 
#   Il permet une gestion simplifiée de bibliothèques logicielles, avec :
#     - Détection automatique du système
#     - Installation personnalisée, complète, par catégorie ou par profil
#     - Export/import de profils de paquets
#     - Vérification de la connexion internet
#     - Vérification de mise à jour du script
#
# Objectif :
#   Fournir une solution simple, réutilisable et partagée pour installer
#   rapidement les logiciels les plus populaires et open-source.
################################################################################

##############################################
# Variables globales
##############################################
scriptNom="UniversalPackageInstaller_Linux"
scriptAlias="upi"
...
# Profils prédéfinis (⚠ LibreAssOS inchangé)
declare -A profilsPredefinis
profilsPredefinis["Minimal"]="VLC Git"
profilsPredefinis["Complet"]="VLC Brave LibreOffice GIMP OBS Studio Blender VirtualBox"
profilsPredefinis["Gamer"]="Steam Lutris Discord"
profilsPredefinis["Bureautique"]="LibreOffice Thunderbird Nextcloud"
profilsPredefinis["Developpeur"]="Git GitHub VSCode Docker"
profilsPredefinis["Creatif"]="GIMP Blender Inkscape Krita"
profilsPredefinis["LibreAssOS"]="Nextcloud LibreOffice Thunderbird VLC"
...
function verifierConnexionInternet {
    ...
    if ping -q -c 1 -W 2 google.com >/dev/null; then
        ...
        echo "❌ Aucune connexion Internet détectée. Impossible de continuer."
        exit 1
    fi
}
...
function verifierMiseAJour {
    ...
    if [[ -n "$versionEnLigne" && "$versionEnLigne" != "$scriptVersion" ]]; then
        ...
        echo "👉 Téléchargez-la depuis GitHub pour bénéficier des dernières améliorations."
    else
        echo "✅ Vous utilisez la dernière version ($scriptVersion)."
    fi
}
...
function detecterSysteme {
    case "$(uname -s)" in
        Linux*)   systeme="Linux" ;;
        MINGW*|CYGWIN*|MSYS*) systeme="Windows" ;;
        *)        systeme="Inconnu" ;;
    esac
...
##############################################
# Bibliothèque de paquets (ajout simplifié)
##############################################
...
function bibliothequePaquets {
    ...
    if [ "$action" == "Ajouter" ]; then
        ...
        paquetsCommande["$nom"]="$commande"
    fi
}

##############################################
# Définition des paquets populaires Open Source
##############################################

# VLC
bibliothequePaquets "Ajouter" \
  "VLC" "Multimedia" \
  "Lecteur multimédia libre capable de lire presque tous les formats audio/vidéo." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "Brave" "Navigateur" \
  "Navigateur web rapide, axé sur la confidentialité et intégré avec Tor." \
  "" \
...
bibliothequePaquets "Ajouter" \
  ...
  "Suite bureautique libre et complète (traitement de texte, tableur, présentation...)." \
  "" \
...
bibliothequePaquets "Ajouter" \
  ...
  "Client officiel GitHub en ligne de commande." \
  "" \
...
bibliothequePaquets "Ajouter" \
  ...
  "Éditeur de code populaire de Microsoft, extensible et multiplateforme." \
  "" \
...
bibliothequePaquets "Ajouter" \
  ...
  "Plateforme de virtualisation légère pour les applications conteneurisées." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "Steam" "Jeux" \
  "Plateforme de jeux vidéo avec support Linux (Proton)." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "Lutris" "Jeux" \
  "Gestionnaire open-source pour installer et lancer des jeux Linux/Windows." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "GIMP" "Graphisme" \
  "Éditeur d’images open-source, alternative à Photoshop." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "Inkscape" "Graphisme" \
  "Éditeur vectoriel libre, alternative à Adobe Illustrator." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "Krita" "Graphisme" \
  "Logiciel de peinture numérique libre, destiné aux artistes." \
  "" \
...
bibliothequePaquets "Ajouter" \
  ...
  "Logiciel open-source de streaming et d’enregistrement vidéo." \
  "" \
  "sudo apt install -y obs-studio"

# Thunderbird
bibliothequePaquets "Ajouter" \
  "Thunderbird" "Bureautique" \
  "Client de messagerie open-source développé par Mozilla." \
  "" \
...
bibliothequePaquets "Ajouter" \
  "Nextcloud" "Cloud" \
  "Solution libre d’hébergement et synchronisation de fichiers." \
  "" \
...
function main {
    clear
    echo "==============================================="
    echo " 🚀 Bienvenue dans $scriptNom ($scriptAlias)"
    echo " Auteur : $scriptCreateur"
